...
/**
   ...
   * tags.
   */
test("Subscripts and superscripts in markdown should render in HTML.", async ({ page }) => {
  ...
  const actuals = paragraphs.map(simplifyInnerHtmlWhitespace).filter(p => p.startsWith("Example"))
  const expected = [
